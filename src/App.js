...
function exportCSV(months, channels, campaigns, funnel) {
  ...
  const a = document.createElement("a"); a.href = url; a.download = "marketing-roi-report.csv"; a.click();
...
function exportPDF(months, channels, campaigns, funnel) {
  ...
  const enrichedChannels = channels.map(ch => ({ ...ch, roi: ch.spend ? Math.round(((ch.revenue-ch.spend)/ch.spend)*100) : 0 }));
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}body{font-family:'DM Sans',sans-serif;background:#fff;color:#1a1a2e;padding:48px;max-width:1000px;margin:0 auto}
  h1{font-size:28px;font-weight:800;margin-bottom:4px;color:#0B0F1A}h2{font-size:18px;font-weight:700;margin:32px 0 14px;color:#0B0F1A;border-bottom:2px solid #06D6A0;padding-bottom:6px}
...
function useDragReorder(items, setItems) {
  ...
  const onDrop = (e,idx) => {
    ...
    if(from===null||from===idx){setDraggedOver(null);return;}
    const u=[...items]; const[m]=u.splice(from,1); u.splice(idx,0,m);
    setItems(u); dragIdx.current=null; setDraggedOver(null);
  };
  const onDragEnd = () => { dragIdx.current=null; setDraggedOver(null); };
  // Touch-based reorder (up/down buttons for mobile)
...
function MobileReorder({ drag, idx, total }) {
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:2, flexShrink:0 }}>
      <button onClick={()=>drag.moveUp(idx)} disabled={idx===0}
        style={{ background:"none", border:"none", color: idx===0?C.border:C.muted, fontSize:14, cursor: idx===0?"default":"pointer", padding:0, lineHeight:1 }}>▲</button>
      <button onClick={()=>drag.moveDown(idx)} disabled={idx===total-1}
        style={{ background:"none", border:"none", color: idx===total-1?C.border:C.muted, fontSize:14, cursor: idx===total-1?"default":"pointer", padding:0, lineHeight:1 }}>▼</button>
    </div>
  );
}
  ...
  return (
    <div style={{ background:C.card, borderRadius:16, border:`1px solid ${C.border}`, marginBottom:20, overflow:"hidden" }}>
      <div style={{ padding:"16px 16px 12px", display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:8 }}>
        <div><h3 style={{ margin:0, fontSize:15, fontWeight:700, color:C.text }}>{title}</h3>
          {subtitle && <p style={{ margin:"3px 0 0", fontSize:12, color:C.dim }}>{subtitle}</p>}</div>
        {action}
      </div>
...
function AddBtn({ onClick, label }) {
  return <button onClick={onClick} style={{ background:C.accentDim, color:C.accent, border:`1px solid ${C.accent}33`, borderRadius:8, padding:"6px 14px", fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>{label}</button>;
}
function RemoveBtn({ onClick }) {
  return <button onClick={onClick} style={{ background:"rgba(239,71,111,.1)", color:C.danger, border:"none", borderRadius:6, width:28, height:28, cursor:"pointer", fontSize:16, lineHeight:1, flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center" }}>×</button>;
}
  ...
  return (
    <div>
      <div style={{ background:`linear-gradient(135deg, ${C.accent}10, ${C.info}08)`, border:`1px solid ${C.accent}20`,
        borderRadius:14, padding: isMobile?"14px 16px":"18px 24px", marginBottom:20, display:"flex", gap:12, alignItems:"flex-start" }}>
        <span style={{ fontSize:20, flexShrink:0, marginTop:1 }}>💡</span>
        <div>
          <div style={{ fontSize:13, fontWeight:700, color:C.text, marginBottom:4 }}>Enter your marketing data to generate your dashboard</div>
          <div style={{ fontSize:12, color:C.muted, lineHeight:1.6 }}>
            Fill in each tab below. Sample data is pre-loaded. {isMobile ? "Use ▲▼ to reorder rows." : "Drag to reorder rows."} Click <strong style={{ color:C.accent }}>Generate Dashboard</strong> when ready.
          </div>
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:4, marginBottom:20, flexWrap:"wrap" }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            padding: isMobile?"8px 12px":"10px 20px", fontSize: isMobile?12:13, fontWeight:600, cursor:"pointer",
            ...
            fontFamily:"'DM Sans',sans-serif", transition:"all .2s", display:"flex", alignItems:"center", gap:4,
                ...
                <div key={i} style={{ background:C.cardAlt, borderRadius:10, padding:12, border:`1px solid ${C.border}` }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                    ...
                    <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                      <MobileReorder drag={monthsDrag} idx={i} total={months.length} />
              ...
              isMobile ? (
                <div key={i} style={{ background:C.cardAlt, borderRadius:10, padding:12, border:`1px solid ${C.border}` }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                    ...
                    <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                      <MobileReorder drag={channelsDrag} idx={i} total={channels.length} />
      ...
      {tab === "campaigns" && (
        <SectionCard title="Campaign Details" subtitle="Add individual campaigns"
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {campaigns.map((cp, i) => (
              <div key={i} {...(!isMobile ? { draggable:true, onDragStart:e=>campaignsDrag.onDragStart(e,i), onDragOver:e=>campaignsDrag.onDragOver(e,i),
                onDragLeave:campaignsDrag.onDragLeave, onDrop:e=>campaignsDrag.onDrop(e,i), onDragEnd:campaignsDrag.onDragEnd } : {})}
                style={{ background: campaignsDrag.draggedOver===i?C.dragOver:C.cardAlt, borderRadius:10, padding:12,
                  border: campaignsDrag.draggedOver===i?`1px solid ${C.dragBorder}`:`1px solid ${C.border}`, transition:"all .15s" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                    {!isMobile && <DragHandle />}
                    <span style={{ fontSize:12, fontWeight:600, color:C.muted }}>{cp.name || `Campaign ${i+1}`}</span>
                  </div>
                  <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                    {isMobile && <MobileReorder drag={campaignsDrag} idx={i} total={campaigns.length} />}
                    <RemoveBtn onClick={() => removeRow(campaigns, setCampaigns, i)} />
                  </div>
                </div>
              ...
              isMobile ? (
                <div key={i} style={{ background:C.cardAlt, borderRadius:10, padding:12, border:`1px solid ${C.border}` }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                    ...
                    <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                      <MobileReorder drag={funnelDrag} idx={i} total={funnel.length} />
      ...
      <div style={{ display:"flex", justifyContent:"center", marginTop:16 }}>
        <button onClick={onLaunch} style={{
          ...
          borderRadius:12, padding: isMobile?"14px 36px":"15px 52px", fontSize: isMobile?15:16, fontWeight:700, cursor:"pointer",
          fontFamily:"'DM Sans',sans-serif", boxShadow:`0 4px 24px ${C.accent}33`, width: isMobile?"100%":"auto",
...
function KPI({ title, value, change, icon, color=C.accent }) {
  ...
  return (
    <div style={{ background:C.card, borderRadius:14, padding:"18px 16px", border:`1px solid ${C.border}`, position:"relative", overflow:"hidden", minWidth:0 }}>
      ...
      <div style={{ fontSize:24, fontWeight:700, color:C.text, letterSpacing:"-.02em" }}>{value}</div>
      {change !== null && (
...
function ExportMenu({ months, channels, campaigns, funnel }) {
  ...
  const btnBase = { display:"flex", alignItems:"center", gap:10, width:"100%", padding:"11px 16px",
    background:"transparent", border:"none", color:C.text, fontSize:13, fontWeight:500,
    cursor:"pointer", borderRadius:8, fontFamily:"'DM Sans',sans-serif", textAlign:"left", transition:"background .15s" };
  return (
    <div ref={ref} style={{ position:"relative" }}>
      ...
      }}>📤 Export <span style={{fontSize:10,opacity:.7}}>▼</span></button>
      {open && (
        <div style={{ position:"absolute", top:"calc(100% + 6px)", right:0, width:220,
          ...
          <button style={btnBase} onMouseEnter={e=>e.currentTarget.style.background=C.cardHover}
            onMouseLeave={e=>e.currentTarget.style.background="transparent"}
            onClick={()=>{exportPDF(months,channels,campaigns,funnel);setOpen(false);}}>
            <span style={{fontSize:18}}>📄</span><div><div style={{fontWeight:600}}>Export as PDF</div><div style={{fontSize:11,color:C.dim}}>Print-ready report</div></div>
          </button>
          <div style={{height:1,background:C.border,margin:"2px 8px"}} />
          <button style={btnBase} onMouseEnter={e=>e.currentTarget.style.background=C.cardHover}
            onMouseLeave={e=>e.currentTarget.style.background="transparent"}
            onClick={()=>{exportCSV(months,channels,campaigns,funnel);setOpen(false);}}>
            <span style={{fontSize:18}}>📊</span><div><div style={{fontWeight:600}}>Export as CSV</div><div style={{fontSize:11,color:C.dim}}>For Excel / Sheets</div></div>
          </button>
        </div>
      )}
    </div>
...
function DashboardView({ months, channels, campaigns, funnel }) {
  const { isMobile, isTablet } = useScreen();
  const [tab, setTab] = useState("overview");

  const enrichedMonths = months.map(m => ({ ...m, roas:m.spend?+(m.revenue/m.spend).toFixed(2):0, cpl:m.leads?+(m.spend/m.leads).toFixed(2):0 }));
  const totalSpend=sum(months,"spend"), totalRev=sum(months,"revenue"), totalLeads=sum(months,"leads");
  ...
  const smallChartH = isMobile ? 200 : 200;

  const tabs = [
    { id:"overview", label:"Overview" },
    { id:"channels", label: isMobile?"Channels":"Channel Performance" },
    { id:"campaigns", label:"Campaigns" },
    { id:"funnel", label: isMobile?"Funnel":"Funnel Analysis" },
  ];
  ...
  return (
    <div>
      <div style={{ display:"flex", gap:4, marginBottom:24, flexWrap:"wrap" }}>
        {tabs.map(t => (
          <button key={t.id} onClick={()=>setTab(t.id)} style={{
            padding: isMobile?"8px 12px":"9px 18px", fontSize: isMobile?12:13, fontWeight:600, cursor:"pointer",
      ...
      {tab==="overview" && (
        <>
          ...
          <div style={isMobile ? {
            ...
            {[
              { title:"Total Revenue", value:fmt$(totalRev), change:revChange, icon:"💰", color:C.accent },
              { title:"Total Spend", value:fmt$(totalSpend), change:spendChange, icon:"📊", color:C.info },
              { title:"Blended ROAS", value:`${blendedROAS}x`, change:roasChange, icon:"🎯", color:C.purple },
              { title:"Total Leads", value:totalLeads.toLocaleString(), change:leadChange, icon:"👥", color:C.warn },
              { title:"Avg CPL", value:`$${avgCPL}`, change:cplChange, icon:"⬇️", color:C.cyan },
            ].map((k,i) => (
              <div key={i} style={isMobile ? { minWidth:160, flex:"0 0 160px", scrollSnapAlign:"start" } : {}}>
                <KPI {...k} />
              </div>
            ))}
          </div>
            ...
            <div style={{ background:C.card, borderRadius:16, padding: isMobile?"16px 12px":26, border:`1px solid ${C.border}` }}>
              ...
              <ResponsiveContainer width="100%" height={chartH}>
                <AreaChart data={enrichedMonths} margin={{ top:5, right:10, left: isMobile?-10:10, bottom:5 }}>
                  ...
                  <XAxis dataKey="month" tick={{fill:C.dim,fontSize:isMobile?9:11}} axisLine={{stroke:C.border}} />
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                  ...
                  <div style={{gridColumn:"1/-1"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      <div style={{flex:1,height:5,background:C.border,borderRadius:3,overflow:"hidden"}}><div style={{width:`${Math.min(ch.roi/7,100)}%`,height:"100%",background:ch.color,borderRadius:3}}/></div>
                      <span style={{color:ch.color,fontSize:14,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{ch.roi}%</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <>
          ...
          <div style={{ display:"grid", gridTemplateColumns: isMobile?"repeat(2,1fr)":"repeat(4,1fr)", gap:12 }}>
            <div style={{background:C.card,borderRadius:14,padding: isMobile?14:22,border:`1px solid ${C.border}`,textAlign:"center"}}>
              ...
              <div style={{color:C.accent,fontSize: isMobile?26:32,fontWeight:800}}>{activeCampaigns}</div>
            </div>
            <div style={{background:C.card,borderRadius:14,padding: isMobile?14:22,border:`1px solid ${C.border}`,textAlign:"center"}}>
              <div style={{color:C.dim,fontSize:10,marginBottom:4,textTransform:"uppercase"}}>Avg ROI</div>
              <div style={{color:C.info,fontSize: isMobile?26:32,fontWeight:800}}>{enrichedCampaigns.length?Math.round(enrichedCampaigns.reduce((s,c)=>s+c.roi,0)/enrichedCampaigns.length):0}%</div>
            </div>
            <div style={{background:C.card,borderRadius:14,padding: isMobile?14:22,border:`1px solid ${C.border}`,textAlign:"center"}}>
              ...
              <div style={{color:C.dim,fontSize:11,marginTop:2}}>{bestCampaign?`${bestCampaign.roi}% ROI`:""}</div>
            </div>
            <div style={{background:C.card,borderRadius:14,padding: isMobile?14:22,border:`1px solid ${C.border}`,textAlign:"center"}}>
              <div style={{color:C.dim,fontSize:10,marginBottom:4,textTransform:"uppercase"}}>Pipeline</div>
              <div style={{color:C.warn,fontSize: isMobile?26:32,fontWeight:800}}>{fmt$(sum(campaigns,"revenue"))}</div>
            </div>
          </div>
        </>
      )}
        ...
        <div style={{ display:"grid", gridTemplateColumns: isMobile?"1fr":"1fr 1fr", gap:16 }}>
          <div style={{ background:C.card, borderRadius:16, padding: isMobile?20:28, border:`1px solid ${C.border}` }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:5, alignItems:"center" }}>
              {funnel.map((f,i)=>{
                const w=30+70*(1-i/(funnel.length-1||1)), op=.4+.6*(1-i/(funnel.length-1||1));
                return (
          ...
          <div style={{ background:C.card, borderRadius:16, padding: isMobile?20:28, border:`1px solid ${C.border}` }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              {funnel.slice(1).map((f,i)=>{
                const prev=funnel[i].value, rate=pct(f.value,prev);
                return (
                  <div key={i}>
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
                      <span style={{ fontSize:11, color:C.muted }}>{funnel[i].stage} → {f.stage}</span>
                      <span style={{ fontSize:12, fontWeight:700, color:C.accent, fontFamily:"'JetBrains Mono',monospace" }}>{rate}%</span>
                    </div>
            ...
            {funnel.length>=2 && (
              <div style={{ marginTop:20, padding: isMobile?14:18, borderRadius:11, background:"rgba(6,214,160,.05)", border:`1px solid rgba(6,214,160,.12)` }}>
                ...
                <div style={{ display:"flex", alignItems:"baseline", gap:6, flexWrap:"wrap" }}>
                  <span style={{ fontSize: isMobile?24:28, fontWeight:800, color:C.accent, fontFamily:"'JetBrains Mono',monospace" }}>
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'DM Sans',sans-serif", color:C.text, opacity:ready?1:0, transition:"opacity .5s" }}>
      ...
      <div style={{ padding: isMobile?"14px 16px":"24px 36px", borderBottom:`1px solid ${C.border}`,
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <div style={{ width:30, height:30, borderRadius:8, background:`linear-gradient(135deg,${C.accent},${C.info})`,
            display:"flex", alignItems:"center", justifyContent:"center", fontSize:15, fontWeight:800, color:C.bg, flexShrink:0 }}>M</div>
          <div>
            <h1 style={{ margin:0, fontSize: isMobile?16:21, fontWeight:800, letterSpacing:"-.02em" }}>Marketing ROI Dashboard</h1>
            <p style={{ margin:"1px 0 0", color:C.dim, fontSize: isMobile?10:12 }}>
              {mode==="input" ? "Enter your marketing data" : "Executive Performance Report"}
            </p>
          </div>
        </div>
          ...
          {mode==="dashboard" && (
            <>
              <button onClick={()=>setMode("input")} style={{
                ...
                padding: isMobile?"6px 10px":"8px 16px", fontSize: isMobile?11:12, fontWeight:600, cursor:"pointer", fontFamily:"'DM Sans',sans-serif",
              }}>✏️ Edit</button>
      ...
      <div style={{ padding: `24px ${pad} 48px` }}>
        {mode==="input" ? (
          ...
          <DashboardView months={months} channels={channels} campaigns={campaigns} funnel={funnel} />
        )}
      </div>
    </div>
  );
